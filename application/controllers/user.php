...
class user extends CI_Controller { 
    ...
    function sendmessage(){
        ...
        if (isset($auth_key)) {
            ...
            if($this->common_model->isValidApi($auth_key)) {
                ...
                $device_id = $this->getdeviceid($receiver_id);
                $data = $this->dcm->save_message($auth_key, $post_array);
                 
                //push
                 //$this->dcm->send_notifications($device_id, $message, "");  
   ...
   function getall_users(){
    ...
    function deleteMessage()
    {
        ...
        if (isset($auth_key)) {
            ...
            if($this->common_model->isValidApi($auth_key)) {
                $data = $this->dcm->delete_message($auth_key, @$this->request_array);
            }else{
                $data = $this->_set_failes("In valid API-KEY");
            }
        } else {
 ...
 function delete_all(){
        ...
        if (isset($auth_key)) {
            ...
            if($this->common_model->isValidApi($auth_key)) {
                $data = $this->dcm-> delete_all($auth_key);
              }else{
                $data = $this->_set_failes("In valid API-KEY");
            }
        } else {
    ...
    function update_online_status(){
        ...
        if($numrows > 0){
            ...
            foreach($result as $re){
                ...
                $last_updated_time = $re->last_update;
                $current_time = date("Y-m-d H:i:s");
                
                if($last_updated_time != "0000-00-00 00:00:00"){
                    
                    $last_updated_time = strtotime($last_updated_time);
                    $current_time = strtotime($current_time);
                    $diff = round(abs($current_time - $last_updated_time) / 60,2);
                    if($diff > 5){
                        ...
                        $this->db->where('UserId', $re->UserId);
                        $this->db->update('User', $data);
                    }
                    //$r .= round(abs($current_time - $last_updated_time) / 60,2);
                    //$r .= "<br/>";
                }
            ...
            //mail("vnair.abhinand@gmail.com","My subject",$r);
