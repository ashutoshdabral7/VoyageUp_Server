...
//		   }
    ...
    function get_allusers($auth_key, $post_data){
            $userid = $this->getIdByAuthKey($auth_key);
            $current_user_details = $this->get_userdetails($userid, $auth_key);
            
            $this->db->where("UserId !=", $userid);
            $this->db->where("network_id", $current_user_details->network_id);
    ...
    function getalllatestmessages($auth_key){
        ...
        $this->db->where("u.online_status",1);
    ...
    function delete_message($api_key,$post_array)
    {
        
        $userid = $this->getIdByAuthKey($api_key);
        $sender_id=$post_array["user_id"];
        ...
        {
            $data['status'] = $this->failed_string;
            $data['message'] = 'Error deleteing';
            return $data;
        }
    }
	function delete_all($api_key)
    {
        
        $userid = $this->getIdByAuthKey($api_key);
       
        $this->db->where("(MessageReceiver = $userid) OR (MessageSender = $userid)");
        $delete= $this->db->delete('Messages');
        if($delete==TRUE)
        {
            $data['status'] = $this->success_string;
